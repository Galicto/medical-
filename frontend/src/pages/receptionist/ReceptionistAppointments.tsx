...
import { Search, Calendar, Clock, Edit2 } from 'lucide-react';
...
const ReceptionistAppointments = () => {
    ...
    useEffect(() => {
        receptionistModuleApi.getAppointments()
            .then(res => {
                const formatted = res.data.map((appt: any) => ({
                    id: appt.id,
                    patient: appt.patient?.user?.name || 'Unknown',
                    doctor: appt.doctor?.user?.name || 'Unassigned',
    ...
    const filtered = appointments.filter(a => {
        ...
        const matchesSearch = a.patient.toLowerCase().includes(search.toLowerCase()) || a.doctor.toLowerCase().includes(search.toLowerCase());
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold" style={{ color: 'var(--text-primary)' }}>Appointments Registry</h1>
                    <p className="text-sm mt-1" style={{ color: 'var(--text-faint)' }}>Manage {appointments.length} overall appointments</p>
                </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
                <div className="relative flex-1">
                    ...
                    <input type="text" placeholder="Search by patient or doctor..." value={search} onChange={e => setSearch(e.target.value)}
                ...
                <div className="overflow-auto">
                    <table className="w-full text-sm">
                        ...
                        <tbody>
                            {filtered.length === 0 ? (
                                <tr>
                                    <td colSpan={7} className="text-center py-8" style={{ color: 'var(--text-faint)' }}>No appointments found.</td>
                                </tr>
                            ) : filtered.map((appt, i) => {
                                ...
                                return (
                                    <motion.tr key={appt.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 + i * 0.03 }}
                                        ...
                                        <td className="px-5 py-4 font-medium" style={{ color: 'var(--text-secondary)' }}>{appt.patient}</td>
                                        <td className="px-5 py-4" style={{ color: 'var(--text-muted)' }}>{appt.doctor}</td>
                                        ...
                                        <td className="px-5 py-4">
                                            <button className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium transition-all theme-btn-secondary" style={{ color: 'var(--text-primary)' }}>
